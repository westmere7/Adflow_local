    ...
    subs: [
      ...
      { id: 'features-list', title: 'Powerful Features', body: `
        ...
        <ul style="line-height: 1.6; padding-left: 20px; margin-top: 12px; margin-bottom: 12px;">
          ...
          <li style="margin-bottom: 8px;"><b>Fully Local, No Accounts</b>: Nothing to sign in to and nothing uploaded. Work autosaves locally, travels as portable <code>.flow</code> files, and the whole application runs on your own machine — with no internet access required.</li>
      ...
      `},
      { id: 'how-its-run', title: 'Where your work is kept', body: `
        <p>Adflow is a desktop application for Windows and macOS. There is nothing to install alongside it — the browser engine it needs is inside the download — and nothing to sign into.</p>
        <p><b>Your work lives on this machine, in this copy of the app.</b> Autosave, <b>Open Recent</b>, the base project and remembered placements are all held locally. They do not sync, and they do not follow you to another computer.</p>
        <p>So to move a project — to another machine, or to a colleague — save it as a file: <b>File → Save → Save to File (.flow)</b>. A <code>.flow</code> carries the whole project, embedded images and all, and opens in any copy of Adflow.</p>
        <p>The same goes for anything you want to be certain of keeping. Autosave is a safety net against a crash, not a backup.</p>
      `},
  ...
  },
  {
    ...
    subs: [
      { id: 'technical-stack-intro', title: 'Introduction', body: `
        <p>Technical Stack details the code architecture, data structure, and layout mechanics for engineering and IT administrators. It covers the vanilla script loading sequence, global state schemas, CSS clip-path masking, local persistence, and how the application is packaged as a desktop build.</p>
      ...
      `},
      { id: 'tech-deployment', title: 'Packaging (Electron desktop build)', body: `
        <p>Adflow's application layer is a static site: HTML, CSS, vanilla JavaScript and a handful of vendored binaries, with no server-side code at all. It is delivered as a <b>desktop application</b>, packaged with Electron.</p>
        <p><b>The shell:</b> <code>electron/</code> is roughly 300 lines across three files — a window and menu policy (<code>main.js</code>), a read-only loopback HTTP server (<code>static-server.js</code>) and a preload that exposes a single <code>window.adflowDesktop</code> marker. Nothing in <code>scripts/</code>, <code>styles.css</code> or the three HTML pages was changed to make the desktop build work, which is deliberate: the shell is replaceable, the application is not.</p>
        <p><b>Why it serves itself over HTTP:</b> the app cannot run from <code>file://</code>. Every ad preview is an <code>&lt;iframe srcdoc&gt;</code> sandbox and export spawns a <code>blob:</code> Worker that <code>importScripts()</code> the vendored JSZip — both get opaque origins under <code>file://</code> and are blocked. Serving over <code>http://127.0.0.1</code> gives the renderer the environment the app was written against, and Chromium treats loopback as a secure context, which is what keeps <code>showSaveFilePicker</code> and WebCodecs video export available. The port is pinned at <b>47823</b> because local storage is keyed to the origin; a random port each launch would present an empty workspace every time. A genuine conflict falls back to the next free port and reports it in a dialog rather than appearing to have lost the user's work.</p>
        <p><b>Build:</b> <code>npm run build:win</code> / <code>build:mac</code> run the two Node generator scripts, then electron-builder produces an unpacked portable folder in <code>dist/</code> — not an installer. It needs no admin rights, writes nothing to the registry, and is removed by deleting the folder.</p>
        <p><b>No outbound requests:</b> every library (JSZip, iro.js, mediabunny, gifenc, HarfBuzz) and every font (the brand fonts and the portals' Inter and Outfit) ships in the application, so it works on a machine with no internet access at all.</p>
      `}
    ]
  }
];
...
const CHANGELOG_DATA = [
  {
    ...
    items: [
      "Dragging Is Now Essentially Free: moving a layer no longer rebuilds anything. The layers you are dragging, their selection outline and the snap guides move directly, and the full redraw waits until you let go — a drag that previously cost 62 rebuilds now costs none. Anything that changes more than a position falls back to the normal redraw on its own: holding Alt to duplicate, dragging onto another canvas, dropping a photo onto an existing layer, or moving a mask.",
      "Dragging and Zooming Are Smoother: moving an element or zooming the workspace rebuilt the entire editor — every canvas, every panel, the rulers — once for each movement the mouse reported. A modern mouse reports position many times faster than that work can finish, so the backlog grew as you dragged and the motion stuttered, worse on bigger projects. Redraws are now batched to at most one per screen refresh: a short drag that previously cost 62 full rebuilds now costs one. Nothing looks different — it just keeps up.",
      "Adflow Is a Desktop Application for the People Who Make Ads: the managed static-hosting config has been removed. The container image, its web-server configuration, their double-click launchers and the operator guide stay in the repository, so ITS can review the app and host it on RMIT infrastructure without installing anything. Nothing about the application changed: the code that runs in a browser tab is the same code inside the desktop build, unmodified.",
      "The Manual Says the Thing That Actually Matters: the old “Desktop app or hosted page” section compared two ways of running Adflow. With the desktop build the way everyone makes ads, it has been replaced by “Where your work is kept” — your projects live on this machine, in this copy of the app; they do not sync and they do not follow you to another computer; a .flow file is how you move one. Autosave is a safety net against a crash, not a backup.",
      "Technical Stack Documents the Build, Not the Deployment: the Deployment section is now Packaging, covering the Electron shell, why the app serves itself over loopback rather than running from a file, why its port is fixed, and what the build actually produces. The About box matches."
    ]
  },
  {
    version: 'v0.61.0',
